...
export const IPhone13: React.FC<DeviceProps> = ({ children, className = "", scale = 1 }) => {
  return (
    <div className={`relative flex flex-col items-center ${className}`} style={{ transform: `scale(${scale})` }}>
      <div className="relative w-full aspect-[9/19.5] rounded-[3rem] border-[8px] border-gray-900 bg-gray-900 shadow-2xl box-border">
        ...
        <div className="w-full h-full rounded-[2.5rem] overflow-hidden bg-black relative">
          ...
          {/* Notch - iPhone 13 notch is slightly smaller/narrower than older notches but similar to 14 base */}
          <div className="absolute top-0 left-1/2 -translate-x-1/2 h-7 w-28 bg-black rounded-b-2xl z-20 flex justify-center items-start pt-1">
             <div className="w-10 h-1 bg-[#1a1a1a] rounded-full opacity-30" />
          </div>
