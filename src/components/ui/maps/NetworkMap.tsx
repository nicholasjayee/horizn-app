...
const ConnectionLine = ({ start, end }: { start: THREE.Vector3, end: THREE.Vector3 }) => {
   // Optimization: Memoize cloned vector instances to prevent reallocation on re-renders
   const [s, e] = useMemo(() => {
     if (!start || !end) return [null, null];
     return [start.clone().setY(0.2), end.clone().setY(0.2)];
   }, [start, end]);
...
const Packet = ({ start, end }: { start: THREE.Vector3, end: THREE.Vector3 }) => {
  ...
  // Optimization: Cache stochastic values to prevent recalculation on every render
